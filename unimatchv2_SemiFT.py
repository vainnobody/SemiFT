...
def get_parser():
    parser = argparse.ArgumentParser(
        description="UniMatchV2 + SemiFT for Semi-Supervised Semantic Segmentation"
    )
...
def apply_peft(model, cfg):
    ...
    return AdaptModel(peft_config, model)
...
def build_model(cfg):
    ...
    # Force PEFT semantics: always freeze backbone.
...
def main(args, cfg):
    ...
    # Force script identity/config for SemiFT runs.
    cfg["peft"] = "semift"
    ...
    if rank == 0:
        ...
        logger.info("Running UniMatchV2 + SemiFT with backbone forcibly frozen.")
