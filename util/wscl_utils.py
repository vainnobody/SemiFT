...
def generate_unsup_aug_sc(conf_w, mask_w, data_s):
    ...
    for i in range(b):
        ...
        new_conf_w.append(
            (conf_w[i] * augmix_mask + conf_w[(i + 1) % b] * (1 - augmix_mask)).unsqueeze(0)
        )
        new_mask_w.append(
            (mask_w[i] * augmix_mask + mask_w[(i + 1) % b] * (1 - augmix_mask)).unsqueeze(0)
        )
        new_data_s.append(
            (data_s[i] * augmix_mask + data_s[(i + 1) % b] * (1 - augmix_mask)).unsqueeze(0)
        )
...
def generate_unsup_aug_dc(conf_w, mask_w, data_s1, data_s2):
    ...
    for i in range(b):
        ...
        new_data_s.append(
            (data_s1[i] * augmix_mask + data_s2[(i + 1) % b] * (1 - augmix_mask)).unsqueeze(0)
        )
    return torch.cat(new_conf_w), torch.cat(new_mask_w), torch.cat(new_data_s)
