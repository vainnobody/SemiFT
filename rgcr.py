...
def get_parser():
    parser = argparse.ArgumentParser(
        description="RGCR: FixMatch with RVSC augmentation + configurable PEFT"
    )
...
def main(args, cfg):
    ...
    if rank == 0:
        ...
        logger.info(
            "Running RGCR with method=%s, target_modules=%s, freeze_backbone=%s",
            peft_cfg["method"],
    ...
    model = build_model(cfg, peft_cfg)
    ...
    for epoch in range(epoch + 1, cfg["epochs"]):
        ...
        if rank == 0:
            ...
            for class_idx, iou in enumerate(iou_class):
                writer.add_scalar(
                    "eval/%s_IoU" % (CLASSES[cfg["dataset"]][class_idx]), iou, epoch
                )
                writer.add_scalar(
                    "eval/%s_IoU_ema" % (CLASSES[cfg["dataset"]][class_idx]),
                    iou_class_ema[class_idx],
                    epoch,
                )
