...
FixMatch-style implementation with EMA teacher and DPT backbone.
"""
...
from model.semseg.rankmatch_utils import corr_loss
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="RankMatch: Rank-based Consistency Regularization for Semi-Supervised Semantic Segmentation"
    )
...
def main(args, cfg):
    ...
    # Model configuration for different backbone sizes
    model_configs = {
        "small": {
            "encoder_size": "small",
            "features": 64,
    ...
    # RankMatch hyperparameters (hardcoded)
    corr_weight = 0.1  # Weight for correlation consistency loss
    ...
    for epoch in range(epoch + 1, cfg["epochs"]):
        ...
        # RankMatch uses two strong augmentations (s1, s2)
        ...
        for i, (
            ...
            with torch.no_grad():
                pred_u_w_mix, feat_u_w_mix = model_ema(img_u_w_mix)
                pred_u_w_mix = pred_u_w_mix.detach()
                feat_u_w_mix = feat_u_w_mix.detach()
                conf_u_w_mix = pred_u_w_mix.softmax(dim=1).max(dim=1)[0]
                mask_u_w_mix = pred_u_w_mix.argmax(dim=1)

            # Resize feature maps to match prediction size
            H, W = pred_u_w_mix.shape[-2:]
            _, _, h, w = feat_u_w_mix.shape

            # Apply CutMix to strong augmentation images
            ...
            # Detach weak predictions for pseudo-labels (use student predictions, not EMA)
            ...
            # Resize features
            feat_u_w = F.interpolate(
                feat_u_w, size=(H, W), mode="bilinear", align_corners=True
            )
            feat_u_w_mix = F.interpolate(
                feat_u_w_mix, size=(H, W), mode="bilinear", align_corners=True
            )

            # Prepare CutMix pseudo-labels and features
            mask_u_w_cutmixed1 = mask_u_w.clone()
            conf_u_w_cutmixed1 = conf_u_w.clone()
            ignore_mask_cutmixed1 = ignore_mask.clone()
            feat_u_w_cutmixed1 = feat_u_w.clone()

            mask_u_w_cutmixed2 = mask_u_w.clone()
            conf_u_w_cutmixed2 = conf_u_w.clone()
            ignore_mask_cutmixed2 = ignore_mask.clone()
            feat_u_w_cutmixed2 = feat_u_w.clone()

            # Apply CutMix to pseudo-labels and features
            mask_u_w_cutmixed1[cutmix_box1 == 1] = mask_u_w_mix[cutmix_box1 == 1]
            conf_u_w_cutmixed1[cutmix_box1 == 1] = conf_u_w_mix[cutmix_box1 == 1]
            ignore_mask_cutmixed1[cutmix_box1 == 1] = ignore_mask_mix[cutmix_box1 == 1]
            cutmix_box1_expanded = cutmix_box1.unsqueeze(1).expand(feat_u_w.shape)
            feat_u_w_cutmixed1 = torch.where(
                cutmix_box1_expanded == 1, feat_u_w_mix, feat_u_w_cutmixed1
            )
            feat_u_w_cutmixed1 = F.interpolate(
                feat_u_w_cutmixed1, size=(h, w), mode="bilinear", align_corners=True
            )

            mask_u_w_cutmixed2[cutmix_box2 == 1] = mask_u_w_mix[cutmix_box2 == 1]
            conf_u_w_cutmixed2[cutmix_box2 == 1] = conf_u_w_mix[cutmix_box2 == 1]
            ignore_mask_cutmixed2[cutmix_box2 == 1] = ignore_mask_mix[cutmix_box2 == 1]
            cutmix_box2_expanded = cutmix_box2.unsqueeze(1).expand(feat_u_w.shape)
            feat_u_w_cutmixed2 = torch.where(
                cutmix_box2_expanded == 1, feat_u_w_mix, feat_u_w_cutmixed2
            )
            feat_u_w_cutmixed2 = F.interpolate(
                feat_u_w_cutmixed2, size=(h, w), mode="bilinear", align_corners=True
            )

            # Supervised loss
            loss_x = criterion_l(pred_x, mask_x)

            # Unsupervised loss for strong augmentation 1
            ...
            # Unsupervised loss for strong augmentation 2
            loss_u_s2 = criterion_u(pred_u_s2, mask_u_w_cutmixed2)
            loss_mask_s2 = (conf_u_w_cutmixed2 >= cfg["conf_thresh"]) & (
                ignore_mask_cutmixed2 != ignore_index
            )
            ...
            loss_mask_fp = (conf_u_w >= cfg["conf_thresh"]) & (
            ...
            # Rank-based correlation consistency loss
            loss_c_s1 = corr_loss(
                feat_u_w_cutmixed1, feat_u_s1, local_rank, num_landmarks, rank_k
            )
            loss_c_s2 = corr_loss(
                feat_u_w_cutmixed2, feat_u_s2, local_rank, num_landmarks, rank_k
            )

            # Total loss (original RankMatch weighting)
            loss = (
                loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.5
            ) / 2.0 + corr_weight * (loss_c_s1 + loss_c_s2) / 2.0
            ...
            mask_ratio = (
                (conf_u_w >= cfg["conf_thresh"]) & (ignore_mask != ignore_index)
            ...
            if rank == 0:
                ...
                writer.add_scalar(
                    "train/loss_c", (loss_c_s1.item() + loss_c_s2.item()) / 2.0, iters
                )
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info(
                    ...
                    "Loss s: {:.3f}, Loss c: {:.3f}, Loss w_fp: {:.3f}, Mask ratio: {:.3f}".format(
                        i,
